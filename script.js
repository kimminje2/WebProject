...
/* 플러그인 : Flicking 묶음 */
function initFlickingSet(items) {
  items.forEach((item) => initFlicking(item.key, item.selector, item.options || {}, item.pagination || ""));
}

/* 플러그인 : 세로 목록 camera */
function createListCamera(selector) {
  const $list = $(selector).addClass("flicking-viewport").empty();
  return $('<div class="flicking-camera"></div>').appendTo($list);
}

/* 기능 : 도구 페이지 슬라이드 */
function initToolboxFlicking(view) {
  const prefix = view === "all" ? "toolboxAll" : "toolboxStage";
  initFlickingSet(["Dev", "Ai"].map((kind) => ({
    key: `${prefix}${kind}`,
    selector: `#${prefix}${kind}Flicking`,
    pagination: `#${prefix}${kind}Pagination`
  })));
}
/* 플러그인 : Flicking 크기 계산 */
...
/* 도구모음 : 모바일 페이지 레이아웃 사용 여부 확인 */
function isToolboxPagedLayout() {
  return getToolboxPageSize() === 6;
}
...
function renderToolbox() {
  ...
  visibleStages.forEach((item) => {
    ...
    const usePagedLayout = isToolboxPagedLayout();
    ...
    renderToolboxGroup($stage.find(".toolbox-columns"), "개발 프로그램", item.devTools, usePagedLayout ? {
      viewportId: "toolboxStageDevFlicking", paginationId: "toolboxStageDevPagination"
    } : {});

    renderToolboxGroup($stage.find(".toolbox-columns"), "AI", item.aiTools, usePagedLayout ? {
...
function renderRecords() {
  ...
  const $camera = createListCamera("#recordList");
...
function renderPrompts() {
  ...
  const $camera = createListCamera("#promptShelf");

  $("#promptCount").text(`${appData.prompts.length}개`);

  appData.prompts.forEach((prompt, index) => $camera.append(`<article class="prompt-item ${String(index) === $("#editingPromptIndex").val() ? "active" : ""}" data-index="${index}"><strong>${prompt.title}</strong><button class="delete-prompt" type="button" aria-label="${prompt.title} 삭제">X</button></article>`));
  initFlicking("prompts", "#promptShelf", { horizontal: false });
}
