...
/* 플러그인 : 도구 화면의 이전 슬라이드 해제 */
function destroyToolboxFlicking() {
  ["toolboxAllDev", "toolboxAllAi", "toolboxStageDev", "toolboxStageAi"].forEach(destroyFlicking);
}
...
function initRecordDatePicker() {
  ...
  recordDatePicker = window.flatpickr("#recordDateFilter", {
    mode: "range",
    dateFormat: "Y-m-d",
    maxDate: "today",
    ...(locale ? { locale } : {}),
...
function renderAiCompareOptions() {
  const aiTools = appData.tools.filter((tool) => tool.type === "ai" || tool.category === "ai").sort((a, b) => a.name.localeCompare(b.name, "ko"));
...
function renderAiCompare() {
  ...
  $("#aiCompareResult").html([["이름", first?.name || "-", second?.name || "-"], ["특징", first?.summary || "-", second?.summary || "-"], ["비용", costText(first), costText(second)]].map((row) => `<tr><th>${row[0]}</th><td>${row[1]}</td><td>${row[2]}</td></tr>`).join(""));
}
...
function updateRecordFilterOptions() {
  fillSelect($("#recordFieldFilter"), unique([...studyFields, ...appData.records.map((record) => getRecordField(record))]), "전체 분야", $("#recordFieldFilter").val() || "all", (field) => fieldLabels[field] || field);
  fillSelect($("#recordLanguageFilter"), unique([...languageOptions, ...appData.records.map((record) => getRecordLanguage(record))]), "전체 언어", $("#recordLanguageFilter").val() || "all");
}
...
/* 학습일지 : 선택한 날짜 범위와 기록 날짜 비교 */
function matchesRecordDate(record) {
  const selectedDates = recordDatePicker?.selectedDates || [];
  if (!selectedDates.length) return true;
  const recordDate = getRecordDateKey(record);
  if (!recordDate) return false;
  const startDate = getLocalDateKey(selectedDates[0]);
  const endDate = getLocalDateKey(selectedDates[1] || selectedDates[0]);
  return recordDate >= startDate && recordDate <= endDate;
}

/* 학습일지 : 기록 날짜 정렬 */
function compareRecordDate(first, second, order) {
  const firstDate = getRecordDateKey(first);
  const secondDate = getRecordDateKey(second);
  if (!firstDate) return 1;
  if (!secondDate) return -1;
  const comparison = secondDate.localeCompare(firstDate);
...
function renderRecords() {
  ...
  const dateSort = $("#recordDateSort").val() || "newest";
  ...
  const filteredRecords = appData.records
    ...
    .filter((record) => matchesRecordDate(record))
    .sort((first, second) => compareRecordDate(first, second, dateSort));
